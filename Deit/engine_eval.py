...
def train_one_epoch(model: torch.nn.Module, criterion: DistillationLoss,
    ...
    # gather the stats_baseline_cls from all processes
    metric_logger.synchronize_between_processes()
    print("Averaged stats_baseline_cls:", metric_logger)
...
def evaluate(data_loader, model, device, epoch=300):
    ...
    # use small number for fast testing
    total_images = 64
    ...
    for i, b in enumerate(model.module.blocks):
        codes.append(b.attn.code.codes.cpu())
        x = torch.linspace(-7, 7, 400).unsqueeze(-1)
        x_ = x.clone()
        ...
        with torch.no_grad():
            x_ = torch.relu(x_)
            ind_l = torch.minimum(x_//1, torch.tensor(numbers)-1).long()
            ind_r = torch.minimum(x_//1+1, torch.tensor(numbers)-1).long()
            B = len(x)
            code = codes[i].squeeze(0).squeeze(0).repeat(B, 1)
            y = torch.gather(code, -1, ind_l)*(1-x+ind_l) + torch.gather(code, -1, ind_r)*(x-ind_l)
    ...
    if False:
        ...
        # #     print(f"t: {t}")
        # #     # print(f"g: {g}")
            ax = plt.plot(x, np.exp(y), color=colors[i])
        # #     # print("x", x)
        # #     # print("y", y)
        # #     # plt.colorbar(
        # #     # )
        # plt.axis('scaled')
        # # #
        # np.savez(f'baseline_ranges_epoch_{epoch}.npz', *ranges)
        # np.savez(f'baseline_ts_epoch_{epoch}.npz', *ts)
        # #
        plt.xlim([-9, 9])
        ...
        plt.xticks([-7 + i for i in range(0, 14)], fontsize="x-large")
        plt.xlabel("x", fontsize="xx-large")
        plt.ylabel("σ(x)",fontsize="xx-large")
        # plt.legend([f"layer_{j}" for j in range(len(model.module.blocks))]+["y=x"]+["x=b_1"]+["x=d_1"] + ["x=b_2"] + ["x=d_2"], fontsize="x-large", loc="lower right",bbox_to_anchor=(1.13, -0.1))
        # plt.legend([f"layer_{j}" for j in range(len(model.blocks))] + ["y=x"] + ["x=range1"] + ["x=range2"] + [
        #     "x=range3"] + ["x=range4"])
    plt.savefig(f"show/curve_exp.png")
    exit()
        # print("range")
        # print(m)
        # print("t")
        ...
        # y4 = []
        # k = 3
        # # Equiradial Projection
        # # proj = [[np.sin(2 * math.pi * (i + 0.5) / k), np.cos(2 * math.pi * (i + 0.5) / k)] for k in range(k)]
        # # proj = np.array(proj)
        # 
        # for i, b in enumerate(model.module.blocks):
        # # for i, b in enumerate(model.blocks):
        ...
        #     y = scipy.stats_baseline_cls.entropy(y, axis=-1)
    ...
    def highlight_grid(image, grid_indexes, grid_size=14):
        ...
        for grid_index in grid_indexes:
            ...
            a.rectangle([(y * w, x * h), (y * w + w, x * h + h)], fill=None, outline='red', width=6)
    ...
    if show_plot:
        ...
        attns_after = [[] for l in range(len(model.module.blocks))]
        attns_after_top5 = [[] for l in range(len(model.module.blocks))]
        attns_after_bot5 = [[] for l in range(len(model.module.blocks))]

        patch_similarity = [ 0 for l in range(len(model.module.blocks))]
        new = [[] for l in range(len(model.module.blocks))]
        ...
        means_after = [0 for l in range(len(model.module.blocks))]
        mses_after = [0 for l in range(len(model.module.blocks))]
        maxs_after = [0 for l in range(len(model.module.blocks))]
        mins_after = [0 for l in range(len(model.module.blocks))]
        uppers_after = [0 for l in range(len(model.module.blocks))]
        lowers_after = [0 for l in range(len(model.module.blocks))]
        entropy = [0 for l in range(len(model.module.blocks))]
    counts = 0
    ...
    #     # compute output
        ...
        # # # # #     torchvision.utils.save_image(images.cpu(), "show/input.png", normalize=True, nrow=images.shape[0])
            for l in range(len(model.module.blocks)):
                ...
                attn_after = model.module.blocks[l].attn.attn
                # ranges = model.module.blocks[l].attn.ranges.cpu()
                h = model.module.blocks[l].attn.h
                patch_similarity[l] += ((F.normalize(h, dim=-1) @ F.normalize(h, dim=-1).transpose(-2, -1)).mean().item() - 1/h.shape[1])*h.shape[1]/(h.shape[1] - 1)
        #

                B, H, N, N = attn_after.shape
                attn_after = attn_after.cpu().float()
        #
        #        # for vit with cls token
        #         N = N -1
                # cls = attn[:, :, -1:, :-1]
                # attn = attn[:, :, :-1, :-1]
                for k in range(0, N, 34):
                    # print(attn.shape)
                    # normalized attn is too sparse
                    #     print(attn.shape)
                    out = attn_after[:, :, k, :].reshape(-1, int(N ** (1 / 2)), int(N ** (1 / 2))).unsqueeze(1)
                    # print(attn[:, k, :].min(dim=-1, keepdim=True)[0].repeat(1, N).shape)
                    min = [attn_after[:, :, k, :].min(dim=-1, keepdim=True)[0].repeat(1, 1, 1, N * 256).reshape(-1, int(N ** (
                                1 / 2)) * 16, int(N ** (1 / 2)) * 16).unsqueeze(1)] * 2
                    ...
                    fig, axes = plt.subplots(1, H, figsize=(12, 2))
                    for b in range(4, 8, 4):
                        for h in range(H):
                            plt.subplot(1, H, (b-4) // 4 * H + h + 1)
                    ...
                    # cbar_ax = fig.add_axes([0.85, 0.37, 0.02, 0.25])
                    # cbar_ax = fig.add_axes([0.85, 0.37, 0.02, 0.25])
                    # fig.colorbar(im, cax=cbar_ax)
                    # plt.colorbar()
                    plt.tight_layout()

                    plt.savefig(f"show/attn{l}_query{k}.png")
        ...
        #             # save in log space for better histogram visualization, but casue range -inf error
        #             attns_after[l].extend(attn_after[:, h].flatten().cpu().tolist())
        #             attns_after_top5[l].extend(attn_after[:, h].topk(5, -1, largest=True)[0].flatten().cpu().tolist())
        #             attns_after_bot5[l].extend(attn_after[:, h].topk(5, -1, largest=False)[0].flatten().cpu().tolist())
        #
        #             # calculate entropy
        #             entropy[l] += -(attn_after*torch.log(attn_after)).sum(-1).mean().item()
        #
        #
        #         new[l] = attn_after.mean([0,1]).cpu().numpy()
        ...
        #         means_after[l] += attn_after.flatten().mean().item()
        #         mses_after[l] += (attn_after.flatten().std() ** 2).item()
        #         maxs_after[l] = max(maxs_after[l], attn_after.flatten().max().item())
        #         mins_after[l] = min(mins_after[l], attn_after.flatten().min().item())
        #
        #     print(target)
        #     np.savez("new/new.npz", new)
        #     exit()

        if show_plot:
            if counts == total_images // 64 +1:
                break
    ...
    #
        ...
        #         plt.savefig(f"show/baseline_layer_{l}_{counts*2048}.png")
    ...
    patch_similarity = [patch_similarity[l]/counts for l in range(len(model.module.blocks))]
    stds = [(mses[l] / counts)**0.5 for l in range(len(model.module.blocks))]
    means_after = [means_after[l] / counts for l in range(len(model.module.blocks))]
    stds_after = [(mses_after[l] / counts) ** 0.5 for l in range(len(model.module.blocks))]
    entropy= [entropy[l]/counts for l in range(len(model.module.blocks))]
    ...
    uppers_after = [np.percentile(attns_after[l], 99.5) for l in range(len(model.module.blocks))]
    lowers_after = [np.percentile(attns_after[l], 0.5) for l in range(len(model.module.blocks))]
    ...
    patch_similarity = np.array(patch_similarity)
    ...
    print(entropy)
    np.savez(f"baseline_means_epoch_{epoch}.npz", means)
    np.savez(f"baseline_stds_epoch_{epoch}.npz", stds)
    np.savez(f"baseline_maxs_epoch_{epoch}.npz", maxs)
    np.savez(f"baseline_mins_epoch_{epoch}.npz", mins)
    np.savez(f"baseline_uppers_epoch_{epoch}.npz", uppers)
    np.savez(f"baseline_lowers_epoch_{epoch}.npz", lowers)
    np.savez(f"baseline_patch_similarity_epoch_{epoch}.npz", patch_similarity)
    np.savez(f"baseline_entropy_epoch_{epoch}.npz", entropy)
    np.savez(f"baseline_attns_epoch_{epoch}.npz", attns_raw)

    np.savez(f"baseline_means_after_epoch_{epoch}.npz", means_after)
    np.savez(f"baseline_stds_after_epoch_{epoch}.npz", stds_after)
    np.savez(f"baseline_maxs_after_epoch_{epoch}.npz", maxs_after)
    np.savez(f"baseline_mins_after_epoch_{epoch}.npz", mins_after)
    np.savez(f"baseline_uppers_after_epoch_{epoch}.npz", uppers_after)
    np.savez(f"baseline_lowers_after_epoch_{epoch}.npz", lowers_after)
    np.savez(f"baseline_attns_after_epoch_{epoch}.npz", attns_after)
    np.savez(f"baseline_attns_after_top5_epoch_{epoch}.npz", attns_after_top5)
    np.savez(f"baseline_attns_after_bot5_epoch_{epoch}.npz", attns_after_bot5)
    ...
    if show_plot:
        ...
        #     axes.flatten()[l].hist(attns_raw[l], bins=20)
        # plt.savefig(f"show/baseline_layer_{l}.png")
        ...
        for l in range(len(model.module.blocks)):
            # for h in range(6):
            print(f"plotting layer {l}")
            axes.flatten()[l].hist(attns_after[l], bins=50)
            axes.flatten()[l].set_yscale('log')
            axes.flatten()[l].set_ylim([0, 1e8])

        # "supplied range is not finite when using log scale attn score"
        # "plot the numbers in log scale instead"
        plt.savefig(f"show/baseline_layer_after{l}.png")

        # checked and compared, histogram is much clearer than boxplot
        # plt.figure()
        # fig, axes = plt.subplots(6, 2, figsize=(15, 12))
        # for l in range(len(model.module.blocks)):
        #     # for h in range(6):
        #     print(f"plotting layer {l}")
        #     axes.flatten()[l].boxplot(attns_raw[l])
        # plt.savefig(f"show/box_layer_{l}.png")
        ...
        # # # feature histogram
        # plt.figure()
        # plt.hist(feature.flatten().numpy(), bins=20)
